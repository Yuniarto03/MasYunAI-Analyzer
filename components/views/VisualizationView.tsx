...
const COLOR_THEMES: Record<string, string[]> = {
    cyberpunkNight: ['#ff00ff', '#00ffff', '#ffff00', '#00ff00', '#ff5e00', '#7605e2'],
    default: ['#2dd4bf', '#a78bfa', '#f87171', '#fbbf24', '#34d399', '#818cf8', '#fb7185', '#fde047'],
    cosmicFunk: ['#f43f5e', '#ec4899', '#d946ef', '#a855f7', '#7c3aed', '#6366f1'],
    nebula: ['#67e8f9', '#5eead4', '#86efac', '#eab308', '#f97316', '#ef4444'],
    solarFlare: ['#fde047', '#facc15', '#fbbf24', '#f97316', '#ea580c', '#dc2626'],
    oceanicDeep: ['#00b4d8', '#0077b6', '#03045e', '#ade8f4', '#90e0ef', '#caf0f8'],
    sunsetGlow: ['#ff9933', '#ff6666', '#cc3366', '#993366', '#663366', '#333366'],
};
...
const CustomTooltip: React.FC<any> = ({ active, payload, label, data, seriesTotals }) => {
    if (active && payload && payload.length && data && data.length > 0) {
        ...
        return (
            <div className="p-3 rounded-lg shadow-xl border border-purple-500/30 bg-gray-900/80 backdrop-blur-sm w-52">
                <p className="label font-bold text-purple-300 mb-2 truncate">{`${label}`}</p>
                ...
                {payload.length > 1 && (() => {
                    const value1 = payload[0].value as number;
                    const series1Name = payload[0].name;
                    const color1 = payload[0].color || payload[0].stroke || payload[0].fill;

                    const value2 = payload[1].value as number;
                    const series2Name = payload[1].name;
                    const color2 = payload[1].color || payload[1].stroke || payload[1].fill;

                    const totalGap = (typeof value1 === 'number' && typeof value2 === 'number') ? value1 - value2 : null;

                    if (totalGap !== null) {
                        return (
                            <div className="border-t border-gray-700 pt-2 mt-2">
                                <div className="flex justify-between items-center text-xs text-gray-400">
                                    <span className="font-semibold">Series Gap</span>
                                    <span className="font-bold text-gray-200">{totalGap.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                                </div>
                                <div className="text-xs text-gray-500 text-right">
                                    (<span style={{ color: color1 }}>{series1Name}</span>
                                    &nbsp;-&nbsp;
                                    <span style={{ color: color2 }}>{series2Name}</span>)
                                </div>
                            </div>
                        );
                    }
...
const ChartInstance: React.FC<{
    ...
    chartId, state, setState, isActive, onSetActive, onDuplicate, onClear, onMaximize,
    ...
    const handleYAxisAggregatorChange = (field: string, aggregator: AggregatorType) => setState(prev => ({ ...prev, yAxisFields: prev.yAxisFields.map(f => f.field === field ? { ...f, aggregator } : f) }));
    ...
    const handleThemeChange = (themeName: string) => {
        ...
        setState(prev => ({
            ...prev,
            chartOptions: { ...prev.chartOptions, colorTheme: themeName },
    ...
    const handleExport = useCallback(() => {
        ...
        link.setAttribute("download", `chart_${chartId}_data.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    }, [transformedData, chartId]);
    ...
    const renderChart = (isMaximized = false) => {
        ...
        const currentColors = COLOR_THEMES[chartOptions.colorTheme];
        ...
        const commonCartesianProps = { cartesianGrid: chartOptions.showGrid ? <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} stroke="#8b5cf6" /> : null, tooltip: <Tooltip content={<CustomTooltip data={transformedData} seriesTotals={seriesTotals} />} isAnimationActive={false}/>, legend: <Legend {...getLegendProps(chartOptions.legendPosition)} /> };
        ...
        switch (chartType) {
            case 'bar': case 'horizontalBar': {
                const tickFormatter = (tick: any) => {
                    const limit = 15;
                    if (typeof tick === 'string' && tick.length > limit) {
                        return `${tick.substring(0, limit)}...`;
                    }
                    return tick;
                };
                const xAxisEl = chartType === 'horizontalBar' ? <XAxis type="number" stroke={yAxisColor} tick={{ fontSize: 10 }} /> : <XAxis dataKey="name" stroke="#a78bfa" tick={{ fontSize: 10 }} angle={-45} textAnchor="end" interval={0} />;
                const yAxisEl = chartType === 'horizontalBar' ? <YAxis type="category" dataKey="name" stroke="#a78bfa" tick={{ fontSize: 10, width: 120 }} tickFormatter={tickFormatter} interval={0} /> : <YAxis stroke={yAxisColor} tick={{ fontSize: 10 }} />;
                return <BarChart data={transformedData} layout={chartType === 'horizontalBar' ? 'vertical' : 'horizontal'} {...commonProps}> {commonCartesianProps.cartesianGrid} {xAxisEl} {yAxisEl} {commonCartesianProps.tooltip} {commonCartesianProps.legend} {refLine} {yAxisFields.map((field, i) => (<Bar key={field.field} dataKey={field.displayName || field.field} name={field.displayName || field.field} fill={field.color || currentColors[i % currentColors.length]} radius={chartType === 'horizontalBar' ? [0, 4, 4, 0] : [4, 4, 0, 0]} stackId={chartOptions.stackData ? 'a' : undefined}> {chartOptions.showDataLabels && <LabelList dataKey={field.displayName || field.field} position="top" style={{fontSize: 10, fill: '#fff'}} />} </Bar>))} {chartType !== 'horizontalBar' && brush} </BarChart>;
            }
            case 'line': case 'area': {
                const ChartComponent = chartType === 'line' ? LineChart : AreaChart;
                return <ChartComponent data={transformedData} stackOffset={chartOptions.stackData ? "expand" : undefined} {...commonProps}><defs>{chartType === 'area' && <GradientDefs colors={yAxisFields.map((f, i) => f.color || currentColors[i % currentColors.length])} theme={currentColors} />}</defs>{commonCartesianProps.cartesianGrid}<XAxis dataKey="name" stroke="#a78bfa" tick={{ fontSize: 10 }} angle={-45} textAnchor="end" interval={0} /><YAxis stroke={yAxisColor} tick={{ fontSize: 10 }} />{commonCartesianProps.tooltip}{commonCartesianProps.legend}{refLine}{yAxisFields.map((field, i) => { const key = field.displayName || field.field; const color = field.color || currentColors[i % currentColors.length]; const dataProps = { key: field.field, type: "monotone" as const, dataKey: key, stroke: color, name: key, dot: { r: 3, strokeWidth: 2, fill: color }, activeDot: { r: 8, stroke: '#fff', strokeWidth: 2 }, strokeWidth: 2.5, stackId: chartOptions.stackData ? 'a' : undefined, isAnimationActive: false }; if (chartType === 'line') return <Line {...dataProps}>{chartOptions.showDataLabels && <LabelList dataKey={key} position="top" style={{ fontSize: 10, fill: '#fff' }} />}</Line>; return <Area {...dataProps} fill={`url(#${GRADIENT_ID_PREFIX}${i})`} >{chartOptions.showDataLabels && <LabelList dataKey={key} position="top" style={{ fontSize: 10, fill: '#fff' }} />}</Area>;})}{brush}</ChartComponent>;
            }
            case 'pie': {
                if (yAxisFields.length === 0) return null; const yField = yAxisFields[0]; const key = yField.displayName || yField.field; const pieData = transformedData.map(item => ({name: item.name, value: (item as any)[key] as number })); return <PieChart {...commonProps}><Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={isMaximized ? 250 : 120} labelLine={false} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>{pieData.map((_entry, index) => ( <Cell key={`cell-${index}`} fill={currentColors[index % currentColors.length]} />))}</Pie>{commonCartesianProps.tooltip} {commonCartesianProps.legend}</PieChart>;
            }
            case 'scatter': {
                if (yAxisFields.length < 2) return <div className="flex items-center justify-center h-full text-gray-400"><p>Scatter plots require at least two <strong className='text-purple-300'>Y-Axis</strong> fields.</p></div>; const xDataKey = yAxisFields[0].displayName || yAxisFields[0].field; const yDataKey = yAxisFields[1].displayName || yAxisFields[1].field; return (<ScatterChart {...commonProps}>{commonCartesianProps.cartesianGrid}<XAxis type="number" dataKey={xDataKey} name={xDataKey} stroke={yAxisFields[0]?.color || currentColors[0]} tick={{ fontSize: 11 }} /><YAxis type="number" dataKey={yDataKey} name={yDataKey} stroke={yAxisFields[1 % currentColors.length]?.color || currentColors[1 % currentColors.length]} tick={{ fontSize: 11 }} />{commonCartesianProps.tooltip}{commonCartesianProps.legend}<Scatter name={yAxisFields.length > 2 ? yAxisFields[2].displayName : "Data Points"} data={transformedData} fill={yAxisFields[0]?.color || currentColors[0]}>{chartOptions.showDataLabels && <LabelList dataKey="name" position="top" style={{fontSize: 10, fill: '#fff'}} />}</Scatter>{brush}</ScatterChart>);
            }
            case 'radar': {
                return (<RadarChart cx="50%" cy="50%" outerRadius={isMaximized ? "85%" : "80%"} data={transformedData} {...commonProps}><PolarGrid strokeOpacity={0.3} /><PolarAngleAxis dataKey="name" stroke="#a78bfa" tick={{ fontSize: 10 }} /><PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={{ fontSize: 10 }} strokeOpacity={0.5} />{commonCartesianProps.tooltip}{commonCartesianProps.legend}{yAxisFields.map((field, i) => (<Radar key={field.field} name={field.displayName || field.field} dataKey={field.displayName || field.field} stroke={field.color || currentColors[i % currentColors.length]} fill={field.color || currentColors[i % currentColors.length]} fillOpacity={0.6} isAnimationActive={false}/>))}</RadarChart>);
            }
            default: return null;
        }
    };
    ...
    return (
        <Panel>
            <div className="grid grid-cols-12 gap-6" onClick={onSetActive}>
                <div className="col-span-12 xl:col-span-9 bg-gray-800/50 p-4 rounded-lg min-h-[500px] flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                            <button onClick={onClear} title="Clear Chart Configuration" className="p-2 text-gray-300 bg-gray-700/50 hover:bg-gray-600 rounded-md transition-colors"><ClearIcon/></button>
                            <button onClick={onDuplicate} title={`Duplicate to Chart ${chartId === 1 ? 2 : 1}`} className="p-2 text-gray-300 bg-gray-700/50 hover:bg-gray-600 rounded-md transition-colors"><DuplicateIcon/></button>
                            <button onClick={handleExport} title="Export Chart Data as CSV" className="p-2 text-gray-300 bg-gray-700/50 hover:bg-gray-600 rounded-md transition-colors"><ExportIcon /></button>
                            <button onClick={onMaximize} title="Maximize Chart" className="p-2 text-gray-300 bg-gray-700/50 hover:bg-gray-600 rounded-md transition-colors"><ExpandIcon /></button>
                        </div>
                    </div>
                    <div className="flex-1">
                        <ResponsiveContainer width="100%" height="100%">{renderChart(false)}</ResponsiveContainer>
                    </div>
                </div>
                <div className="col-span-12 xl:col-span-3">
                    <details className="space-y-4 bg-gray-900/50 p-4 rounded-lg" open>
                        ...
                        <div className="space-y-4 pt-4 border-t border-gray-700">
                             ...
                             <div> <label className="block text-sm font-medium text-gray-300 mb-1">Color Theme</label> <select value={chartOptions.colorTheme} onChange={e => handleThemeChange(e.target.value)} className="w-full p-2 bg-gray-700 text-gray-200 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 border border-gray-600 shadow-sm"> {Object.keys(COLOR_THEMES).map(theme => <option key={theme} value={theme}>{theme.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</option>)} </select> </div>
        ...
        setState(prev => {
            ...
            if (targetZone === 'xAxis') {
                ...
                const newField: PivotValueFieldConfig = {
                    ...
                    displayName: field,
                    color: COLOR_THEMES[prev.chartOptions.colorTheme][newFieldIndex % COLOR_THEMES[prev.chartOptions.colorTheme].length]
                };
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                {activeChartState.yAxisFields.map((f, i) => (
                                    <div key={f.field} draggable onDragStart={(e) => handleDragStart(e, f.field, 'yAxis', activeEditor)} className="bg-green-900/70 p-2 rounded text-sm text-white w-full flex items-center justify-between">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            <button onClick={(e) => handleOpenColorPicker(f.field, e.currentTarget, activeEditor)} className="w-4 h-4 rounded-full flex-shrink-0 border-2 border-gray-500" style={{ backgroundColor: f.color || COLOR_THEMES[activeChartState.chartOptions.colorTheme][i % COLOR_THEMES[activeChartState.chartOptions.colorTheme].length] }} />
                ...
                <div className="lg:col-span-9 space-y-6">
                    <div className={`p-1 rounded-xl transition-all duration-300 ${activeEditor === 1 ? 'bg-purple-600/20' : 'bg-transparent'}`}>
                        <ChartInstance 
                            chartId={1} 
                            state={chartState1} 
                     ...
                     <div className={`p-1 rounded-xl transition-all duration-300 ${activeEditor === 2 ? 'bg-purple-600/20' : 'bg-transparent'}`}>
                        <ChartInstance 
                            chartId={2} 
                            state={chartState2} 
